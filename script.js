...
function answer(btn, questionNum) {
  ...
  if (isCorrect) {
    ...
    burstHearts(btn);
    // Remove retry hint if showing
    const hint = card.querySelector('.retry-hint');
    if (hint) hint.remove();
    // Show reward
    setTimeout(() => {
    ...
    // Wrong: shake the button, then re-enable everything after animation
    btn.disabled = true;
    btn.classList.add('wrong');

    // Show / update retry hint
    let hint = card.querySelector('.retry-hint');
    if (!hint) {
      hint = document.createElement('p');
      hint.className = 'retry-hint';
      // Insert right above the options list
      const opts = card.querySelector('.quiz-options');
      card.insertBefore(hint, opts);
    }
    const msgs = ['再想想～ 💭', '不對哦，再試一次！🥺', '再努力一下！❤️', '好像不太對呢～ 再想想看？'];
    hint.textContent = msgs[Math.floor(Math.random() * msgs.length)];

    // Re-enable other (non-chosen) options after shake finishes
    setTimeout(() => {
